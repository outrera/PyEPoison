...
def login():
    ...
    if usrdata.authenticateUser() is True:
        Output.write('True')
        return True
    if usrdata.authenticateUser() is False:
        Output.write('False')
        return False
    Output.close()
...
def store_Data(User=False): # User debe ser True o False
    key = Get_key()
    data = file_to_var("/levels/Data.ini")
    if User:
        usrdata = usr_data()
...
def get_tables(input,numt):
    # input indica qué dato deseas consultar
    # (1-id,2-descripcion,3-nombre,4-primario(boolean),5-numero de tablas
    userdata = usr_data()
